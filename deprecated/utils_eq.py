...
class FishEyeImage():
    ...
    def reverse_lens_func_distrot(self, r):
        theta = self.reverse_lens_func(r)
        theta -= (self.a*r**5 + self.b*r**4 +
    ...
    def xy_to_eq(self, x, y, c_ra, c_dec, roll, f, k, c_x=None, c_y=None):
    ...
    def xy_to_eq_distort(self, x, y):
        ...
        pa = np.arctan2(delta_xy[1], delta_xy[0])
        pa[pa < 0] += 2*np.pi
        theta = self.reverse_lens_func_distrot(r)
    ...
    def first_match(self, bin_size=75):
    ...
    def draw_residual(self, dpi, **kwargs):
        stars_ra, stars_dec, matched_stars_theta, matched_stars_pa = self.xy_to_eq(
            self.matched_stars_xy['xcentroid'], self.matched_stars_xy['ycentroid'], c_ra=self.ra, c_dec=self.dec, roll=self.roll, f=self.f, k=self.k)
    ...
    def lens_optimize(self, ra_dec_range=3, roll_range=4, f_range=1, k_range=0.2):
